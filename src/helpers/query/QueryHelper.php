...
class QueryHelper
{
     ...
     * Ex.
      if ($this->from) {
        \src\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'from', $this->from, $user->timezone);
      }
     *
     */
    public static function getQaTasksByOwner(ActiveQuery $query, $objectOwner): void
    {
        $query
            ...
            ->leftJoin(\src\entities\cases\Cases::tableName() . ' as ca', 'ca.cs_id = t_object_id AND t_object_type_id = ' . \modules\qaTask\src\entities\qaTask\QaTaskObjectType::CASE)
