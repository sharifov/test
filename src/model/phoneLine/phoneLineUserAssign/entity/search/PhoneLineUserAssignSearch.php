...
namespace src\model\phoneLine\phoneLineUserAssign\entity\search;
...
class PhoneLineUserAssignSearch extends PhoneLineUserAssign
{
    ...
    public function search($params, Employee $user): ActiveDataProvider
    {
        ...
        if ($this->plus_created_dt) {
            \src\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'plus_created_dt', $this->plus_created_dt, $user->timezone);
        }

        if ($this->plus_updated_dt) {
            \src\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'plus_updated_dt', $this->plus_updated_dt, $user->timezone);
        }
