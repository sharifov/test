<?php

namespace modules\abac\migrations;

use modules\abac\src\entities\AbacPolicy;
use yii\db\Migration;

/**
 * Class m220211_135929_remove_abac_lead_logic_poor_processing
 */
class m220211_135929_remove_abac_lead_logic_poor_processing extends Migration
{
    /**
     * {@inheritdoc}
     */
    public function safeUp()
    {
        if (AbacPolicy::find()->where(['ap_object' => 'lead/lead/logic/poor_processing'])->exists()) {
            $this->delete('{{%abac_policy}}', [
                'IN',
                'ap_object',
                [
                    'lead/lead/logic/poor_processing',
                ]
            ]);
            \Yii::$app->abac->invalidatePolicyCache();
        }
    }

    /**
     * {@inheritdoc}
     */
    public function safeDown()
    {
        if (!AbacPolicy::find()->where(['ap_object' => 'lead/lead/logic/poor_processing'])->exists()) {
            $this->insert('{{%abac_policy}}', [
                'ap_rule_type' => 'p',
                'ap_subject' => '("admin" in r.sub.env.user.roles)',
                'ap_subject_json' => '{"condition":"AND","rules":[{"id":"env_user_roles","field":"env.user.roles","type":"string","input":"select","operator":"in_array","value":"admin"}],"valid":true}',
                'ap_object' => 'lead/lead/logic/poor_processing',
                'ap_action' => '(access)',
                'ap_action_json' => "[\"access\"]",
                'ap_effect' => 1,
                'ap_title' => 'Enable Lead Poor Processing',
                'ap_sort_order' => 50,
                'ap_enabled' => 1,
                'ap_created_dt' => date('Y-m-d H:i:s'),
            ]);

            \Yii::$app->abac->invalidatePolicyCache();
        }
    }
}
