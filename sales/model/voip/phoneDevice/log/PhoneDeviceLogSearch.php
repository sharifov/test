...
class PhoneDeviceLogSearch extends PhoneDeviceLog
{
    ...
    public function search($params, Employee $user): ActiveDataProvider
    {
        ...
        if ($this->pdl_created_dt) {
            \sales\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'pdl_created_dt', $this->pdl_created_dt, $user->timezone);
        }

        if ($this->pdl_timestamp_dt) {
            \sales\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'pdl_timestamp_dt', $this->pdl_timestamp_dt, null);
        }
        if ($this->after_timestamp) {
            \sales\helpers\query\QueryHelper::dateMoreThanByUserTZ($query, 'pdl_timestamp_dt', $this->after_timestamp, null);
        }
