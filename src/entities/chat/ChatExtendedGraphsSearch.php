...
class ChatExtendedGraphsSearch extends ClientChatSearch
{
    ...
    public function stats()
    {
        ...
        foreach ($chatsData as $key => $chat) {
            ...
            if (isset($firstChatAgentMessageResults['cch_id'])) {
                $chatsData[$key]['agent_frt'] = strtotime($firstChatAgentMessageResults['cch_id']['first_msg_date']) - strtotime($chat['cch_created_dt']);
            }


            if (isset($firstMessagesOfChatsResults['cch_id']) && ($chat['cch_status_id'] == ClientChat::STATUS_CLOSED || $chat['cch_status_id'] == ClientChat::STATUS_ARCHIVE)) {
