        ...
        if ($phone && $departmentPhone = $phone->departmentPhoneProject) {
            ...
            if ($upp) {
                ...
                if ($user) {
                    ...
                    Yii::info(
                        'Offline - User (' . $user->username . ') Id: ' . $user->id . ', phone: ' . $incoming_phone_number,
            ...
            if ($conference) {
                if ($conferenceData['StatusCallbackEvent'] === 'conference-end') {
                    ...
                    if ($cPart) {
                        ...
                        Yii::warning(
                            'Not found ConferenceParticipant by callSid: conferenceData - ' . VarDumper::dumpAsString($conferenceData),
...
//          $responseData['data']['response'] = $response;
    ...
    protected function findOrCreateCallByData(RequestDataDTO $requestDataDTO): Call
    {
        ...
        if (!empty($requestDataDTO->Called)) {
            if (PhoneDeviceIdentity::canParse($requestDataDTO->Called)) {
                $agentId = PhoneDeviceIdentity::getUserId($requestDataDTO->Called);
            }
        }
