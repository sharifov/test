...
class EmailIncomingService
{
    ...
    private function getOrCreateLead(int $clientId, string $clientEmail, ?int $projectId, string $internalEmail, int $emailId): ?int
    {
        if ($lead = Lead::find()->findLastActiveSalesLeadByClient($clientId, $projectId)->one()) {
            Yii::error('LeadId: ' . $lead->id . ' Last Action' . $lead->l_last_action_dt);
            return $lead->id;
        }
        ...
        Yii::error('Incoming email. Internal Email: ' . $internalEmail . '. Created Email Id: ' . $emailId . '. | No new lead creation allowed on email.', 'EmailIncomingService');
        return null;
    }
    ...
    private function getOrCreateCaseByExchange(int $clientId, ?int $projectId, string $internalEmail, int $emailId): ?int
    {
        ...
        Yii::error('Incoming email. Internal Email: ' . $internalEmail . '. Created Email Id: ' . $emailId . '. | No new exchange case creation allowed on Email.', 'EmailIncomingService');
        return null;
    }
