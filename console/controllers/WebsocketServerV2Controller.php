...
class WebsocketServerV2Controller extends Controller
{
...
//                    }
                ...
                go(function () use ($server, $request, $redisConfig, $subList) {
                    \Yii::info('New coroutine: start', 'info\ws:actionStart:event:open:coroutineStart');

                    $needSub = [];

                    foreach ($subList as $item) {
                        if (!isset($server->channels[$item])) {
                            $needSub[] = $item;
                        }

                        $server->channelList[$item][$request->fd] = $request->fd;
                    }

                    \Yii::info($needSub, 'info\ws:actionStart:event:open:coroutineStart:needSub');
                    Yii::info($server->channelList, 'info\ws:actionStart:event:open:coroutineStart:globalChannelList');

                    if (empty($needSub)) {
                        return false;
                    }

                    foreach ($needSub as $channel) {
                        go(function () use ($channel, $redisConfig, $server) {
                            Yii::info("Channel {$channel} started", 'info\ws:actionStart:event:open:coroutineStart:subCoroutine:start');
                            $redis = new \Swoole\Coroutine\Redis();
                            ...
                            while (isset($server->channelList[$channel]) && $msg = $redis->recv()) {
                                ...
                                if ($type == 'subscribe') {
                                    ...
                                    if (!empty($server->channelList[$name])) {
                                        foreach ($server->channelList[$name] as $fd) {
                                            try {
                                                $server->push($fd, $info);
                                            } catch (\Throwable $e) {
                            ...
                            if (isset($server->channelList[$channel]) && empty($server->channelList[$channel])) {
                                $redis->unsubscribe([$channel]);
                                unset($server->channelList[$channel]);
                            }

                            Yii::info("Channel {$channel} ended", 'info\ws:actionStart:event:open:coroutineStart:subCoroutine:start');
                        });
                    }

                    \Yii::info('Coroutine: end', 'info\ws:actionStart:event:open:coroutineStart');
