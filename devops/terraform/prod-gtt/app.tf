...
resource "aws_instance" "app" {
  ...
  tags = {
    Name        = "app${count.index}-${var.PROJECT}-${var.NAMESPACE}"
    Environment = var.ENV
    Project     = var.PROJECT
    Ns          = var.NAMESPACE
    Domain      = var.DOMAIN
...
resource "aws_lb_target_group" "api" {
  name     = "api-${var.PROJECT}-${var.NAMESPACE}"
  port     = 9443
  protocol = "HTTPS"
  vpc_id   = var.VPC_ID
}
...
resource "aws_lb_target_group" "ws" {
  name     = "ws-${var.PROJECT}-${var.NAMESPACE}"
  port     = 8080
  protocol = "HTTP"
  vpc_id   = var.VPC_ID
...
resource "aws_lb_target_group" "centrifugo" {
  name     = "centrifugo-${var.PROJECT}-${var.NAMESPACE}"
  port     = 8000
  protocol = "HTTP"
  vpc_id   = var.VPC_ID
