...
<? else: ?>
<a href="<?=\yii\helpers\Url::to(['email/inbox', 'id' => $model->e_id, $dataProvider->pagination->pageParam => ($dataProvider->pagination->page + 1), $dataProvider->pagination->pageSizeParam => $dataProvider->pagination->pageSize])?>" class="view_email" data-email-id="<?=$model->e_id?>">
<? endif; ?>
    ...
    <div class="mail_list">
        ...
        <div class="right">
            <h3><?=Html::encode($model->e_email_from)?> - <?=Html::encode($model->e_email_to)?> <small><?=Yii::$app->formatter->asDatetime(strtotime($model->e_created_dt))?></small></h3>
            <p><?=Html::encode($model->e_email_subject)?></p>
            <p><?/*php echo (\yii\helpers\StringHelper::truncate(\common\models\Email::strip_html_tags($model->e_email_body_html), 150, '...', null, true))*/?></p>
        </div>
    </div>
<?php if($modelEmailView && $modelEmailView->e_id == $model->e_id):?>
    </div>
