...
class ClientNotificationPhoneListSearch extends ClientNotificationPhoneList
{
    ...
    public function search($params, Employee $user): ActiveDataProvider
    {
        ...
        if ($this->cnfl_start) {
            \src\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'cnfl_start', $this->cnfl_start, $user->timezone);
        }

        if ($this->cnfl_end) {
            \src\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'cnfl_end', $this->cnfl_end, $user->timezone);
        }

        if ($this->cnfl_created_dt) {
            \src\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'cnfl_created_dt', $this->cnfl_created_dt, $user->timezone);
        }

        if ($this->cnfl_updated_dt) {
            \src\helpers\query\QueryHelper::dayEqualByUserTZ($query, 'cnfl_updated_dt', $this->cnfl_updated_dt, $user->timezone);
        }
