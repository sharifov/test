...
use src\model\conference\entity\aggregate\log\Logs;
...
class ConferenceLogAggregate
{
    ...
    public function run(): void
    {
        ...
        foreach ($this->events as $event) {
            ...
            if ($event->isConferenceStart() || $event->isConferenceEnd()) {
                $this->logs->add(new \src\model\conference\entity\aggregate\log\LogEvent($event::NAME, null, $event->raw));
            } else {
                $this->logs->add(new \src\model\conference\entity\aggregate\log\LogEvent($event::NAME, $participantId->getValue(), $event->raw));
            }
    ...
    {
        $this->logs->add(new \src\model\conference\entity\aggregate\log\LogParticipants($this->participants->getState()));
    }

    private function logResult(): void
    {
        $this->logs->add(new \src\model\conference\entity\aggregate\log\LogResult($this->participants->getResult()));
    }
