        ...
        <tr>
            ...
            <td class="text-center" style="width: 70px">
                <?php if ($callItem->cl_duration) :?>
                    <span class="badge badge-warning" data-toggle="tooltip" data-html="true" title="Duration: <?=Yii::$app->formatter->asDuration($callItem->cl_duration)?> <?php if ($callItem->queue && $callItem->queue->clq_queue_time) :
                        ?><br> Queue Duration: <?= Yii::$app->formatter->asDuration($callItem->queue->clq_queue_time) ?> <?php
                                                                                                              endif; ?>"><?=gmdate('i:s', $callItem->cl_duration)?></span>
                <?php endif;?>
            </td>
            <td style="width: 80px">
                <?php  if ($callItem->record && $callItem->record->clr_record_sid) :?>
                    <?= CallHelper::displayAudioBtn($callItem->recordingUrl, 'i:s', (int)$callItem->record->clr_duration, $callItem->cl_call_sid) ?>
                <?php  endif;?>
            </td>
